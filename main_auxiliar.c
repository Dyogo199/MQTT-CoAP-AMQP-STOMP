...
#include "funcoes_aleatorio.h"


// Adicione se ainda não existe no seu projeto:
extern int numero_aleatorio(int min, int max);
...
void tratar_mensagem(MensagemWiFi msg) {
    ...
    if (msg.tentativa == 0x9999) {
        if (msg.status == 0) {
            ...
            // --------- COR ALEATÓRIA NÃO-VERDE ----------
            int cor;
            do {
                cor = numero_aleatorio(0, 4); // Sorteia de 0 a 4
            } while (cor == 2); // 2 = verde, pule!

            // 0 = vermelho, 1 = azul, 2 = verde (ignorar), 3 = amarelo, 4 = magenta
            switch (cor) {
                case 0: set_rgb_pwm(PWM_STEP, 0, 0); break;            // vermelho
                case 1: set_rgb_pwm(0, 0, PWM_STEP); break;            // azul
                case 3: set_rgb_pwm(PWM_STEP, PWM_STEP, 0); break;     // amarelo
                case 4: set_rgb_pwm(PWM_STEP, 0, PWM_STEP); break;     // magenta
            }

            render_on_display(buffer_oled, &area);
            sleep_ms(1000);

            // Volta ao verde padrão
